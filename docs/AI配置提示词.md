...
6. 第一次配置使用 `SEMANTIC_SCHOLAR_DAYS=7`、`SEMANTIC_SCHOLAR_MAX_RESULTS_PER_QUERY=5`、`KEYWORD_BOOST_WEIGHT=1.5`、`SEND_EMPTY=false`。启用默认经典文献补位，并保持 3 篇、最低 20 次引用、最低相关度 0.65、影响力前 25% 的默认值。除非我另有要求，不要启用 Crossref。
...
B. 可直接粘贴的 Variables 表：必须包含下列每一项，保留多行值的换行。
   ...
   CLASSIC_FALLBACK_NUM
   CLASSIC_FALLBACK_CANDIDATES_PER_QUERY
   CLASSIC_FALLBACK_MIN_CITATIONS
   CLASSIC_FALLBACK_RELEVANCE_THRESHOLD
   CLASSIC_FALLBACK_IMPACT_TOP_FRACTION
   MAX_PAPER_NUM
   SEND_EMPTY
   LANGUAGE
